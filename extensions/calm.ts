...
const CALM_WORKING_PULSE = ["·", "•", "●", "•"];
const calmWorkingIndicator = (ui: ExtensionUIContext) => ({
  // Use the same muted theme color as the Thinking... / Working... label.
  frames: CALM_WORKING_PULSE.map((symbol) => ui.theme.fg("muted", symbol)),
  intervalMs: 180,
});
...
// Keep the default Calm preference in Pi's per-user home rather than anchoring it
// to this package checkout. FM_* overrides remain available for Firstmate homes.
const piAgentDir = process.env.PI_CODING_AGENT_DIR || resolve(homedir(), ".pi", "agent");
const root = resolve(piAgentDir, "..");
...
export default function (pi: ExtensionAPI) {
  ...
  // Track one logical agent run and the currently executing tools. Thinking is the
  // interval between tool executions; any active tool switches the same built-in row
  // to "working...".
  let agentRunActive = false;
  let activeToolExecutions = 0;
  let calmIndicatorConfigured = false;

  const applyWorkingPresentation = (ui: ExtensionUIContext): void => {
    const calmActive = calmPresentationIsActive();
    if (calmActive) {
      if (!calmIndicatorConfigured) {
        ui.setWorkingIndicator(calmWorkingIndicator(ui));
        calmIndicatorConfigured = true;
      }
      ui.setWorkingMessage(activeToolExecutions > 0 ? "Working..." : "Thinking...");
    } else {
      if (calmIndicatorConfigured) {
        ui.setWorkingIndicator();
        calmIndicatorConfigured = false;
      }
      ui.setWorkingMessage();
    }
    ui.setWorkingVisible(!calmActive || agentRunActive);
  };
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    removeTerminalInputHandler = ctx.ui.onTerminalInput((data) => {
      ...
      setTimeout(() => {
        ...
        // rows that consult Calm live in render(), need without appending anything to
        // the transcript.
        repaintCalmToolRows();
        ctx.ui.setStatus("firstmate-calm", undefined);
      }, 0);
  ...
  pi.registerCommand("calm", {
    description: "Toggle the conversation-only transcript presentation.",
